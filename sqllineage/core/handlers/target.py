...
class TargetHandler(NextTokenBaseHandler):
    ...
    def _handle(self, token: Token, holder: SubQueryLineageHolder) -> None:
        if isinstance(token, Function):
            ...
            holder.add_write(
                Table.of(token.token_first(skip_cm=True), self.table_metadata)
            )
            ...
            holder.add_write(Table.of(token.left, self.table_metadata))
            holder.add_read(Table.of(token.right, self.table_metadata))
            ...
            if token.token_first(skip_cm=True).ttype is Number.Integer:
                ...
                holder.add_write(Table.of(token, self.table_metadata))
